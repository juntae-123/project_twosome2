...
const NoticeInpPage = () => {
  ...
  const notice = NoticeData.find((v) => v.id === 1);

  if (!notice) return <div>해당 공지사항을 찾을 수 없습니다.</div>;

  return (
    <div className=" w-[1212px]  mx-auto   flex flex-col gap-10 items-start ">
      <Title text="공지사항" />
      <h1 className="text-4xl font-bold mt-4 mb">{notice.text}</h1>
      <p className="text-base whitespace-pre-wrap">{notice.date}</p>
      <hr className="border-[#ebebeb]  w-[100%]" />
      <p className="text-base whitespace-pre-wrap">{notice.detail}</p>
    </div>
  );
};
